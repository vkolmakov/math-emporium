...
const Location = ({
    ...
    onLocationTitleClick,
    getLocationTitleLink,
}) => (location) => {
    const Title = ({ location, onClick, getLocationLink }) => {
        if (
            typeof onClick === "function" &&
            typeof getLocationLink === "function"
        ) {
            return (
                <Link
                    onClick={() => onClick(location)}
                    to={getLocationLink(location)}
                    className="name">
    ...
    const Phone = (phone) => (
        <p className="phone">
            <a className="location-info-link" href={`tel:${phone}`}>
                {phone}
            </a>
        </p>
    );
    const Email = (email) => (
        <p className="email">
            <a className="location-info-link" href={`mailto:${email}`}>
                {email}
            </a>
        </p>
    );
    const Description = (description) => {
        const toDescriptionParagraph = (tok, i) => (
            <p key={`${location.id}-description-${i}`} className="description">
                {tok}
            </p>
        );
    ...
    return (
        <li
            key={location.id}
            className="locations-info-location-container"
            style={
                shouldDisplayImageBackground
            ...
            <div
                className="location-info-location-overlay"
                style={
                    shouldDisplayImageBackground
                        ? backgroundPictureOverlayStyle()
                        : {}
                ...
                <div className="location-info-location-data">
                    <Title
                        location={location}
                        onClick={onLocationTitleClick}
                        getLocationLink={getLocationTitleLink}
                    />
