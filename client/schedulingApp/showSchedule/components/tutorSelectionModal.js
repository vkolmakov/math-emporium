...
class TutorSelectionModal extends Component {
    constructor(props) {
        ...
        if (selectedOpenSpotInfo.tutors) {
            tutorOptions =
                selectedOpenSpotInfo.tutors.length > 1
    ...
    render() {
        ...
        const getTutorOptions = (state) =>
            state.tutors.map((t) => ({ label: t.name, value: t.id }));
                ...
                switch (err.data.status) {
                    case 401: {
                        this.props.displayMessageModal({
                            ...
                            redirectToAfterClosing: "/auth/signin",
                        });
                        this.props.signoutUser();
                        break;
                    }
                    case 422: {
                        const shouldRedirectToProfile = (message) =>
        ...
        return (
            <Modal
                ...
                <h3 className="modal-subtitle">
                    {getAppointmentInfoDisplay(selectedOpenSpotInfo)}
                </h3>

                <FilterControls
                    options={getTutorOptions(this.state)}
                    disableNullOption
                    testId={TEST_ID.MODAL_TUTOR_SELECT}
                    label="Select your tutor"
                    currentValue={
                        this.state.requestedTutor
                            ? this.state.requestedTutor.id
                            : null
                    }
                ...
                <div className="textarea-input-group">
                    <label htmlFor="textarea-additional-comments">
                        Additional comments
                    </label>
                    <textarea
