...
function processEvent(event) {
    ...
    const formatTime = (t) => moment(t, TIMESTAMP_FORMAT).format(TIMESTAMP_DISPLAY_FORMAT);
    ...
    return {
        ...
        data: event.data
            ? `${event.data.location.name}: ${event.data.course.code} on ${formatTime(event.data.time)}`
            : '',
    };
}
