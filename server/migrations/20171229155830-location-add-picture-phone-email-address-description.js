...
const newColumns = (Sequelize) => ([
    { name: 'pictureLink', options: { type: Sequelize.STRING, allowNull: true } },
    ...
    { name: 'description', options: { type: Sequelize.STRING, allowNull: true } },
]);
...
module.exports = {
    ...
    down: (queryInterface, Sequelize) => queryInterface.sequelize.transaction((t) => {
        ...
        return Promise.all(newColumns(Sequelize).map(removeWithTransaction));
    }),
};
