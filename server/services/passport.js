...
const azureAdOAuth2Login = new AzureAdOAuth2Strategy(
    azureOptions,
    async (_aT, _rT, params, _p, done) => {
        ...
        try {
            const userWithStatus = await User.findOrCreate({
                where: { email },
...
export default {
    ...
    authenticate: {
        azure: () =>
            passport.authenticate("azure_ad_oauth2", OAUTH_CALLBACK_OPTIONS),
    },
